...
#define BUTTON_PIN_ENTER 6  // Connect enter button to pin 6
#define BUTTON_PIN_UP 7     // Connect up button to pin 7
#define BUTTON_PIN_DOWN 8   // Connect down button to pin 8

// Assign notes to their buzzer frequencies
#define C3 131
#define D3 147
#define E3 165
#define G3 196
...
// User inputted distance and time
int miles = 0;
int decimal = 0;
int minutes = 8;
int seconds = 0;
int paceSeconds = 0;
float timerSeconds = 0.0;
int elapsedTime = 0;
...
void playWindowsStartup() {
  int notes[] = { 523, 659, 784, 880, 784, 659, 523 }; // C5, E5, G5, A5, G5, E5, C5
  int durations[] = { 300, 300, 400, 500, 400, 300, 700 };

  for (int i = 0; i < 7; i++) {
    tone(BUZZER_PIN, notes[i], durations[i]);
    delay(durations[i] * 1); // Adds a soft transition effect
  }
}

// Function for Game Boy sound effect
void playGameBoyStartup() {
  int notes[] = { C3, G3, E4, C4, G4, C5 };
  int durations[] = { 300, 300, 300, 300, 300, 500 };
...
// Function for Mario Kart start sound effect
void playMarioKartStart(int count) {
  int notes[] = { C4, D3, E3, C4 };
  int durations[] = { 200, 200, 200, 500 };
  tone(BUZZER_PIN, notes[count], durations[count]);
  delay(durations[count]);
}


// Update distance screen
void updateDistance() {
  lcd.clear();
  lcd.setCursor(0, 0);
...
void getIMUInterval() {
  int error;
  double dT;
  unsigned long t_now = currentTime;
  ...
  currentInterval = (float)elapsedTime / steps * 1000;
}
// Setup function
void setup() {
  lcd.begin(16, 2);
...
// Main loop
void loop() {
  EnterButtonState = digitalRead(BUTTON_PIN_ENTER);
  UpButtonState = digitalRead(BUTTON_PIN_UP);
  ...
  switch (appState) {
    ...
    case countdown:
      ...
      lcd.print("Run Start!");
      lcd.setCursor(7, 1);
      lcd.print(String(countDownTimer) + "...");
      playMarioKartStart(3-countDownTimer);
      countDownTimer--;
      if (countDownTimer == 0) {
        appState = timer;
    ...
    case timer:
      ...
      if (timerSeconds <= 0 || EnterButtonState == HIGH) {
        appState = runOver;
      }
      delay(50);
      break;
